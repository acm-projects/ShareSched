  ...
  Future<void> callApi(String? sP, String? cN, String? cY, String sN) async {
    ...
    if (isLoaded) {
      ...
      final classM = ClassModel(
        ...
        catalogYear: cY,
        sectionNumber: sN,
        start_time: (scheduleM?.data?.first.meetings?.first.startTime)
            ?.toIso8601String(),
        end_time:
            (scheduleM?.data?.first.meetings?.first.endTime)?.toIso8601String(),
        building:
            scheduleM?.data?.first.meetings?.first.location?.building as String,
        room: scheduleM?.data?.first.meetings?.first.location?.room as String,
        map_uri:
            scheduleM?.data?.first.meetings?.first.location?.mapUri as String,
